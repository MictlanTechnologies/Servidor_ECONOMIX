...
public class AIController {
    ...
    public ResponseEntity<AISummaryResponse> getSummary(
        ...
        Integer userId = currentUserService.getRequiredUserId();
        ...
        List<AISummaryResponse.SavingGoalProgress> metas = aiDataService.getAhorros(userId).stream().map(this::toGoal).toList();
    ...
    public ResponseEntity<SpendForecastResponse> predictSpend(
        ...
        Integer userId = currentUserService.getRequiredUserId();
        horizonDays = (horizonDays == 7 || horizonDays == 30) ? horizonDays : 7;
        LocalDate end = to == null ? LocalDate.now() : to;
        LocalDate from = end.minusDays(90);
        List<Gasto> gastos = aiDataService.getGastos(userId, from, end, categoryId);
        if (gastos.size() < 10) {
            return ResponseEntity.ok(SpendForecastResponse.builder().status("INSUFFICIENT_DATA").recommendations(recommendations()).horizonDays(horizonDays).trainedUntil(end).explanation("No hay suficientes observaciones para estimar predicción robusta.").build());
        }
        ForecastService.ForecastResult fr = forecastService.forecastSpend(gastos, from, end, horizonDays);
        return ResponseEntity.ok(SpendForecastResponse.builder().status("OK").horizonDays(horizonDays).trainedUntil(end).expectedSpend(fr.expected()).lower95(fr.lower95()).upper95(fr.upper95()).explanation(fr.explanation()).build());
    }

    @GetMapping("/predict/budget-risk")
    public ResponseEntity<BudgetRiskResponse> predictBudgetRisk(@RequestParam Integer month, @RequestParam Integer year) {
        Integer userId = currentUserService.getRequiredUserId();
        ...
        for (Presupuesto p : presupuestos) {
            BigDecimal consumido = sum(gastosMes.stream().filter(g -> Objects.equals(g.getIdCategoria(), p.getIdCategoria())).map(g -> nz(g.getMontoGasto())).toList());
            BigDecimal pct = p.getMontoMaximo() == null || p.getMontoMaximo().compareTo(BigDecimal.ZERO) <= 0 ? BigDecimal.ZERO : consumido.multiply(BigDecimal.valueOf(100)).divide(p.getMontoMaximo(), 2, RoundingMode.HALF_UP);
            ...
            items.add(BudgetRiskResponse.BudgetRiskItem.builder().idPresupuesto(p.getIdPresupuesto()).idCategoria(p.getIdCategoria()).categoria(p.getCategoria()).montoMaximo(scale2(nz(p.getMontoMaximo()))).montoConsumido(scale2(consumido)).porcentajeConsumido(scale2(pct)).proyeccionFinMes(scale2(proyectado)).riesgo(riesgo).build());
        }
    ...
    public ResponseEntity<AnomalyResponse> getAnomalies(
        ...
        for (Gasto g : gastos) {
            ...
            BigDecimal z = mad.compareTo(BigDecimal.ZERO) == 0 ? BigDecimal.ZERO : monto.subtract(median).abs().divide(mad, 6, RoundingMode.HALF_UP).multiply(new BigDecimal("0.6745"));
            if (z.compareTo(new BigDecimal("3.5")) >= 0) {
                anomalies.add(AnomalyResponse.AnomalyItem.builder().idGasto(g.getIdGastos()).fecha(g.getFechaGastos()).articulo(g.getArticuloGasto()).idCategoria(g.getIdCategoria()).monto(scale2(monto)).robustZScore(scale2(z)).build());
            }
        }

        return ResponseEntity.ok(AnomalyResponse.builder().status("OK").median(scale2(median)).mad(scale2(mad)).anomalies(anomalies).build());
    }
    ...
    public ResponseEntity<ConfidenceIntervalResponse> inferCiMean(
        ...
        Integer userId = currentUserService.getRequiredUserId();
        List<BigDecimal> sample = aiDataService.getGastos(userId, from, to, categoryId).stream().map(Gasto::getMontoGasto).filter(Objects::nonNull).toList();
        if (sample.size() < 10) {
            return ResponseEntity.ok(ConfidenceIntervalResponse.builder().status("INSUFFICIENT_DATA").recommendations(recommendations()).metricDefinition("Media de gasto por transacción").sampleSize(sample.size()).confidence(confidence).build());
        }
        InferentialStatsService.CIResult ci = inferentialStatsService.confidenceIntervalMean(sample, confidence.doubleValue());
        return ResponseEntity.ok(ConfidenceIntervalResponse.builder().status("OK").metricDefinition("Media de gasto por transacción").sampleSize(ci.n()).mean(scale2(ci.mean())).confidence(confidence).lower(scale2(ci.lower())).upper(scale2(ci.upper())).build());
    }
    ...
    public ResponseEntity<HypothesisTestResponse> compareMeans(@RequestBody CompareMeansRequest request) {
        ...
        Integer userId = currentUserService.getRequiredUserId();
        List<BigDecimal> sampleA = aiDataService.getGastos(userId, request.getFromA(), request.getToA(), request.getCategoryId()).stream().map(Gasto::getMontoGasto).filter(Objects::nonNull).toList();
        List<BigDecimal> sampleB = aiDataService.getGastos(userId, request.getFromB(), request.getToB(), request.getCategoryId()).stream().map(Gasto::getMontoGasto).filter(Objects::nonNull).toList();

        if (sampleA.size() < 10 || sampleB.size() < 10) {
            return ResponseEntity.ok(HypothesisTestResponse.builder().status("INSUFFICIENT_DATA").recommendations(recommendations()).nA(sampleA.size()).nB(sampleB.size()).conclusion("Se requieren al menos 10 observaciones por periodo para aplicar Welch t-test.").build());
        }
        InferentialStatsService.WelchTestResult res = inferentialStatsService.welchTTest(sampleA, sampleB, alpha.doubleValue());
        return ResponseEntity.ok(HypothesisTestResponse.builder().status("OK").nA(res.nA()).nB(res.nB()).meanA(scale2(res.meanA())).meanB(scale2(res.meanB())).differenceMeans(scale2(res.diff())).t(scale2(res.t())).df(scale2(res.df())).pValue(scale2(res.p())).conclusion(res.conclusion()).build());
    }

    @PostMapping("/chat")
    public ResponseEntity<AiChatResponse> chat(@Valid @RequestBody AiChatRequest request) {
        return ResponseEntity.ok(aiChatService.chat(currentUserService.getRequiredUserId(), request));
    }

    private AISummaryResponse insufficientSummary() {
        return AISummaryResponse.builder().status("INSUFFICIENT_DATA").recommendations(recommendations()).topArticulos(List.of()).topCategorias(List.of()).progresoMetasAhorro(List.of()).build();
    }

    private List<String> recommendations() {
        return List.of("Captura al menos 10 transacciones por categoría y periodo.", "Registra fecha exacta y artículo del gasto para mejorar tendencia y anomalías.", "Mantén actualizados presupuestos y metas de ahorro para análisis de riesgo.");
    }
    ...
    private AISummaryResponse.SavingGoalProgress toGoal(Ahorro a) {
        ...
        BigDecimal pct = meta.compareTo(BigDecimal.ZERO) <= 0 ? BigDecimal.ZERO : ahorrado.multiply(BigDecimal.valueOf(100)).divide(meta, 2, RoundingMode.HALF_UP);
        return AISummaryResponse.SavingGoalProgress.builder().idAhorro(a.getIdAhorro()).nombreObjetivo(a.getNombreObjetivo()).meta(scale2(meta)).montoAhorrado(scale2(ahorrado)).porcentajeProgreso(scale2(pct)).build();
    }

    private BigDecimal nz(BigDecimal value) { return value == null ? BigDecimal.ZERO : value; }
    private BigDecimal add(BigDecimal a, BigDecimal b) { return nz(a).add(nz(b)); }
    ...
    private BigDecimal div(BigDecimal a, BigDecimal b) { return (b == null || b.compareTo(BigDecimal.ZERO) == 0) ? BigDecimal.ZERO : a.divide(b, 6, RoundingMode.HALF_UP); }
    ...
    private BigDecimal scale2(BigDecimal v) { return nz(v).setScale(2, RoundingMode.HALF_UP); }
}
